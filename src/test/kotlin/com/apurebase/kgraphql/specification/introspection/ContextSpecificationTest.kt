...
class ContextSpecificationTest {
  ...
  fun `query resolver should not return context param`() {
    ...
    val response = deserialize(schema.executeBlockingGetOne("{__schema{queryType{fields{args{name}}}}}"))
  ...
  fun `mutation resolver should not return context param`() {
    ...
    val response = deserialize(schema.executeBlockingGetOne("{__schema{mutationType{fields{args{name}}}}}"))
    println("response: $response")
